...
import { CONFIGURATION, ConfigurationInfo } from '@kubernetes-dashboard/channels';
import { ConfigurationManager } from '/@/manager/configuration-manager';

@injectable()
export class ConfigurationDispatcher
  extends AbsDispatcherObjectImpl<void, ConfigurationInfo>
  implements DispatcherObject<void>
{
  constructor(@inject(ConfigurationManager) private configurationManager: ConfigurationManager) {
    super(CONFIGURATION);
  }

  getData(): ConfigurationInfo {
