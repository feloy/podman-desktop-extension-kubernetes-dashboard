...
import { type ForwardConfig, type PortForwardableResource, type PortMapping, WorkloadKind } from '/@common/model/port-forward';
...
export class PortForwardConnectionService {
  ...
  protected async getPod(name: string, namespace: string): Promise<V1Pod> {
    const details = this.contextsManager.getResourceDetails(
      this.kubeConfig.currentContext, 'pods', name, namespace);
   ...
   */
  protected async getDeployment(_name: string, _namespace: string): Promise<V1Deployment> {
    // TODO: implement this
    return {} as V1Deployment;
  }
   ...
   */
  protected async getService(_name: string, _namespace: string): Promise<V1Service> {
    // TODO: implement this
    return {} as V1Service;
  }
...
//    const labelSelector = `${selectorKey}=${matchLabels[selectorKey]}`;
    ...
    //const podName = this.requireNonUndefined(podList.items[0]?.metadata?.name, 'Found undefined Pod name.');
...
//    const podList = await this.kubeClient.listNamespacedPod(namespace, undefined, labelSelector);
//    const pod = this.requireNonUndefined(podList.items[0], 'Found undefined Pod.');
...
//    const targetRemotePort = this.getTargetPort(service, pod, forward.remotePort);
    ...
    return {
      ...
      forward: {
        ...
        remotePort: 0, // TODO targetRemotePort,
      } as PortMapping,
    };
  }
  ...
  protected getTargetPort(service: V1Service, pod: V1Pod, port: number): number {
    ...
    const targetPort = this.requireNonUndefined(seekPort?.targetPort, 'Failed to map remote port to target Service port.');
