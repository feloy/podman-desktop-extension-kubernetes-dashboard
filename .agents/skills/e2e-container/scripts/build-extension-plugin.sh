...
trace() { echo "[$(date +%H:%M:%S)] $*"; }

trace "Starting pnpm install..."
CI=true pnpm install
trace "pnpm install done"

trace "Starting pnpm build (rpc → channels → webview+extension)..."
pnpm build
trace "pnpm build done"

PLUGINS_DIR=tests/playwright/tests/playwright/output/kubernetes-dashboard-tests/plugins/extension

trace "Copying build artifacts to $PLUGINS_DIR..."
mkdir -p "$PLUGINS_DIR"
...
trace "Patching package.json with isomorphic-ws..."
...
trace "Running pnpm install --prod in plugin dir..."
pnpm --dir "$PLUGINS_DIR" install --prod
trace "pnpm install --prod done"
